...
export default async function handler(req, res) {
  try {
    ...
    // ---- /api/participation-stats ---- ユーザーの累計参加回数を取得
    if (sub === "participation-stats") {
      const session = await getSession(req);
      if (!session?.username) return res.status(401).json({ error: "認証が必要です" });
      ...
      try {
        // ユーザーの累計参加回数を取得（運転手・添乗員別）
        const statsQuery = await query(`
          SELECT 
            kind,
            COUNT(*) AS count,
            MIN(decided_at) AS first_participation,
            MAX(decided_at) AS last_participation
          FROM selections
          WHERE username = $1
          GROUP BY kind
        `, [session.username]);
        ...
        for (const row of statsQuery.rows) {
          if (row.kind === "driver") {
            stats.driver = Number(row.count) || 0;
          } else if (row.kind === "attendant") {
            stats.attendant = Number(row.count) || 0;
          }
        }

        // 全体の統計を設定
        if (totalQuery.rows?.[0]) {
          stats.total = Number(totalQuery.rows[0].total_count) || 0;
          stats.unique_events = Number(totalQuery.rows[0].unique_events) || 0;
          stats.first_participation = totalQuery.rows[0].first_participation;
          stats.last_participation = totalQuery.rows[0].last_participation;
        }
        ...
        const monthlyQuery = await query(`
          ...
          FROM selections
          WHERE username = $1
          GROUP BY DATE_TRUNC('month', decided_at)
          ORDER BY month DESC
          LIMIT 12
        `, [session.username]);

        const monthly = monthlyQuery.rows.map(row => ({
          month: row.month,
        ...
        console.error("[participation-stats] Error:", err);
        return res.status(500).json({ error: "統計の取得に失敗しました: " + (err?.message || String(err)) });
      }
    }
